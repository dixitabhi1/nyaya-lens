...
import { BadgeCheck, BookOpenText, Star } from "lucide-react";
...
import { fallbackLawyerDetail } from "@/lib/nyayasathi-data";
import {
  getLawyerProfile,
  type LawyerDetail,
} from "@/services/api";
        ...
        <div className="mx-auto max-w-4xl">
          <Card className="rounded-[28px] border-slate-200 bg-white/95 shadow-xl shadow-slate-200/40">
            <CardContent className="space-y-4 p-8 text-center">
              <p className="font-display text-4xl font-bold text-slate-950">Loading judge profile</p>
              <p className="text-base text-slate-600">Fetching court details, profile notes, and articles.</p>
            </CardContent>
          </Card>
        </div>
      </div>
    ...
    return (
      <div className="min-h-full bg-background px-4 py-8 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-4xl">
          <Card className="rounded-[28px] border-slate-200 bg-white/95 shadow-xl shadow-slate-200/40">
            <CardContent className="space-y-4 p-8 text-center">
              <p className="font-display text-4xl font-bold text-slate-950">Judge profile not found</p>
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        {usingFallback ? (
          <p className="text-sm text-amber-700">
            Live judge profile data is temporarily unavailable. Showing bundled preview content.
          </p>
        ) : null}
          ...
          <CardContent className="grid gap-6 p-8 lg:grid-cols-[auto_minmax(0,1fr)_auto] lg:items-center">
            ...
            <div className="flex flex-col gap-3">
              <Button asChild className="rounded-full bg-amber-300 text-slate-950 hover:bg-amber-200">
                <Link to="/judge-dashboard">Open Judge Dashboard</Link>
              </Button>
            </div>
          </CardContent>
        </Card>
              ...
              <CardContent className="space-y-4 p-6">
                ...
                <div className="space-y-4">
                  {lawyer.articles.length > 0 ? lawyer.articles.map((article) => (
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4 text-sm leading-7 text-slate-600">
                      Articles will appear here once the judge profile starts publishing court-ready explainers.
                    </div>
                  )}
                </div>
              </CardContent>
            ...
            <Card className="rounded-[28px] border-slate-200 bg-white shadow-lg shadow-slate-200/40">
              <CardContent className="space-y-4 p-6">
                ...
                <div className="space-y-3 text-sm leading-7 text-slate-600">
                  <div><span className="font-semibold text-slate-950">Judicial Service ID:</span> {lawyer.bar_council_id}</div>
                  ...
                  <div><span className="font-semibold text-slate-950">Review Lane:</span> {lawyer.fee}</div>
                  <div><span className="font-semibold text-slate-950">Profile Watchers:</span> {lawyer.follower_count}</div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="rounded-[28px] border-slate-200 bg-white shadow-lg shadow-slate-200/40">
              <CardContent className="space-y-4 p-6">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-2xl font-semibold text-slate-950">Profile Notes</h2>
                </div>
                <div className="space-y-4">
                  {lawyer.reviews.length > 0 ? lawyer.reviews.map((review, index) => (
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4 text-sm leading-7 text-slate-600">
                      Notes will appear once court profile observations are published.
                    </div>
                  )}
                </div>
              </CardContent>
